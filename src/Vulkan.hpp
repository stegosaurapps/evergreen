...
static VkExtent2D
ChooseExtent(const VkSurfaceCapabilitiesKHR &surfaceCapabilities, int width,
             int height) {
  ...
  VkExtent2D extent2D{};
  extent2D.width =
      (uint32_t)std::clamp(width, (int)surfaceCapabilities.minImageExtent.width,
                           (int)surfaceCapabilities.maxImageExtent.width);
  extent2D.height = (uint32_t)std::clamp(
      height, (int)surfaceCapabilities.minImageExtent.height,
