    ...
    void* userData)
{
    ...
    if (severity >= VK_DEBUG_UTILS_MESSAGE_SEVERITY_WARNING_BIT_EXT) {
        std::cerr << "[Vulkan] " << (data && data->pMessage ? data->pMessage : "(no message)") << "\n";
    }
    return VK_FALSE;
}
...
static bool CheckDeviceExtensionSupport(VkPhysicalDevice pd, const std::vector<const char*>& required) {
    uint32_t count = 0;
    ...
    for (auto* r : required) missing.insert(r);

    for (auto& e : exts) missing.erase(e.extensionName);
...
static SwapchainSupport QuerySwapchainSupport(VkPhysicalDevice pd, VkSurfaceKHR surface) {
    SwapchainSupport s{};
    ...
    if (fCount) vkGetPhysicalDeviceSurfaceFormatsKHR(pd, surface, &fCount, s.formats.data());

    uint32_t pCount = 0;
    vkGetPhysicalDeviceSurfacePresentModesKHR(pd, surface, &pCount, nullptr);
    s.presentModes.resize(pCount);
    if (pCount) vkGetPhysicalDeviceSurfacePresentModesKHR(pd, surface, &pCount, s.presentModes.data());
...
bool Renderer::init(const Win32WindowHandles& wh, int width, int height, bool enableValidation) {
    m_enableValidation = enableValidation;
    m_width = width;
    m_height = height;

    if (!createInstance()) return false;
    if (!setupDebug()) return false;
    if (!createSurface(wh)) return false;
...
void Renderer::shutdown() {
    if (!m_device && !m_instance) return; // already down
...
bool Renderer::createInstance() {
    ...
    if (m_enableValidation) {
        if (!CheckInstanceExtensionAvailable(VK_EXT_DEBUG_UTILS_EXTENSION_NAME)) {
            std::cerr << "Missing instance extension: " << VK_EXT_DEBUG_UTILS_EXTENSION_NAME << "\n";
            return false;
        }
        exts.push_back(VK_EXT_DEBUG_UTILS_EXTENSION_NAME);

        if (!CheckInstanceLayerAvailable(kValidationLayer)) {
            std::cerr << "Validation requested but layer not available: " << kValidationLayer << "\n";
            return false;
        }
    }
...
bool Renderer::pickPhysicalDevice() {
    ...
    auto scoreDevice = [&](VkPhysicalDevice pd, uint32_t& outGfx, uint32_t& outPresent) -> int {
        // Must support swapchain extension.
        if (!CheckDeviceExtensionSupport(pd, requiredDevExts)) return -1;
        ...
        for (uint32_t i = 0; i < qCount; ++i) {
            if (qprops[i].queueCount == 0) continue;
    ...
    std::cout << "Queue families: graphics=" << m_graphicsFamily << " present=" << m_presentFamily << "\n";
    return true;
}
...
bool Renderer::createSwapchainViews() {
    ...
    for (size_t i = 0; i < m_swapchainImages.size(); ++i) {
        ...
        VkResult r = vkCreateImageView(m_device, &iv, nullptr, &m_swapchainImageViews[i]);
        if (r != VK_SUCCESS) {
...
bool Renderer::createSyncObjects() {
    ...
    fci.flags = VK_FENCE_CREATE_SIGNALED_BIT; // start signaled so first frame doesn't wait forever
...
bool Renderer::recreateSwapchainIfNeeded() {
    if (!m_swapchainDirty) return true;
    if (m_width <= 0 || m_height <= 0) return true; // minimized; skip
...
void Renderer::drawFrame() {
    ...
    VkResult r = vkAcquireNextImageKHR(
        ...
        m_imageAvailable[fi], VK_NULL_HANDLE,
        &imageIndex);
